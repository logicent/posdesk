...
<?php 
	if ($purchases_invoice_items) : 
        foreach ($purchases_invoice_items as $row_id => $item) :
			echo render('purchases/invoice/item/_form', array('invoice_item' => $item, 'row_id' => $row_id));
        endforeach;
    else : ?>
...
<div class="form-group">
    <div class="col-md-6">
        <button id="del_item" data-url="/accounts/purchases-invoice/del-item" class="btn btn-sm btn-danger" style="display: none;">Delete</button>
        <button id="add_item" data-url="/accounts/purchases-invoice/add-item" class="btn btn-sm btn-default">Add item</button>
    </div>
	<!--
    <div class="col-md-6 text-right">
        <?php Form::hidden('amounts_tax_inc', Input::post('amounts_tax_inc', isset($purchases_invoice) ? $purchases_invoice->amounts_tax_inc : '0')); ?>
...
$(window).on('load', function() 
{
	...
	$('#item_detail').on('change', '#form_item_id', 
		function() { 
			...
			$.ajax({
				type: 'post',
				url: '/accounts/purchases-invoice/get-item',
				data: {
					'item_id': $(this).val(),
				},
