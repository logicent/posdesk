<div class="row">
	<div class="col-md-6">
        <h2> <span class='text-muted'>Report</span>&ensp;
        <span class="btn-group list-filters">
            <?= Html::anchor('sales/invoice', 
                            'All', array('class' => "btn btn-sm btn-default", 'data-status' => '')); ?>
            <?= Html::anchor('sales/invoice/?status=' . Model_Sales_Invoice::INVOICE_STATUS_OPEN, 'Open', 
                            array('class' => 'btn btn-sm btn-default', 'data-status' => Model_Sales_Invoice::INVOICE_STATUS_OPEN)); ?>
            <?= Html::anchor('sales/invoice/?status=' . Model_Sales_Invoice::INVOICE_STATUS_CLOSED, 'Closed', 
                            array('class' => 'btn btn-sm btn-default', 'data-status' => Model_Sales_Invoice::INVOICE_STATUS_CLOSED)); ?>
            <?= Html::anchor('sales/invoice/?status=' . Model_Sales_Invoice::INVOICE_STATUS_CANCELED, 'Canceled', 
...
<table class="table table-hover datatable">
	...
	<tbody>
<?php foreach ($pos_invoices as $item): ?>
		<tr>
