<div class="col-xs-6 col-md-3">
    <div class="thumbnail">
        <!-- <img data-src="holder.js/100%x180" alt="100%x180" src="data:image/svg+xml;base64," data-holder-rendered="true" style="height: 180px; width: 100%; display: block;"> -->
        <img src="{<?= Model_Product_Item::getValue('image_path', $item->item_id) ?>}" alt="..." class="">
        <div class="caption">
            <h5><?= Model_Product_Item::getValue('item_name', $item->item_id) ?></h5>
            <p class="small text-muted"><?= Model_Product_Item::getValue('description', $item->item_id) ?></p>
            <!-- <p><a href="#" class="btn btn-primary" role="button">Button</a> <a href="#" class="btn btn-default" role="button">Button</a></p> -->
        </div>
    </div>
</div>