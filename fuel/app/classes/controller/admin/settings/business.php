...
class Controller_Admin_Settings_Business extends Controller_Authenticate
{
	public function action_index()
	{
		$business = Model_Business::find('first');
		is_null($business) and Response::redirect('admin/settings/business/create');
		...
		if ($val->run())
		{
			...
			try {
				...
				if ($business->save())
				{
					...
					Response::redirect('admin/settings/business/view');
				}
				else
				{
	...
	{
		is_null($id) and Response::redirect('admin/settings/business');

		if ( ! $data['business'] = Model_Business::find($id))
		{
			Session::set_flash('error', 'Could not find business #'.$id);
			Response::redirect('admin/settings/business');
		}

		$this->template->title = "Business";
		$this->template->content = View::forge('settings/business/view', $data);
