...
class Controller_Purchases_Invoice extends Controller_Authenticate
{
	public function action_index()
	{
		$status = Input::get('status');
		if (!$status)
			$status = Model_Purchases_Invoice::INVOICE_STATUS_OPEN;

		$data['purchases_invoices'] = Model_Purchases_Invoice::find('all', 
									array('where' => array(
										array('status', '=', $status)
									), 
		...
		$this->template->content = View::forge('purchases/invoice/index', $data);
	}
	...
	public function action_view($id = null)
	{
		...
		if ( ! $data['pos_invoice'] = Model_Purchases_Invoice::find($id))
		{
	...
	public function action_save($id = null)
	{
		...
		{
			$val = Model_Purchases_Invoice::validate('create');

			if ($val->run())
			{
				$pos_invoice = Model_Purchases_Invoice::forge(array(
					'id' => Input::post('id'),
				...
				try 
				{
					...
					if ($pos_invoice and $pos_invoice->save())
					{
						...
						{
							$pos_invoice_item = Model_Purchases_Invoice_Item::forge(array(
		...
		$pos_invoice_item = Model_Purchases_Invoice_Item::forge();
	...
	public function action_delete($id = null)
	{
		...
		if ($pos_invoice = Model_Purchases_Invoice::find($id))
		{
